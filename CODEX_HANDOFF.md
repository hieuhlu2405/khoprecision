...
- Chu du an chot khong sua thu tu cua phieu cu. Da tao ban thay the `supabase-sql/20260723_fix_shipment_print_line_order_future_only.sql`, CHUA CHAY LIVE: chi them cot/index `shipment_line_no`, khong UPDATE/backfill dong cu; wrapper tao chuyen ghi so 1-2-3 theo dung thu tu payload trong cung giao dich; wrapper dieu chinh ghi lai thu tu sau sua. Neu khong gan du thu tu, toan bo tao/sua chuyen rollback, khong luu nua chung. Core tao/dieu chinh khong mo quyen goi truc tiep.
...
- Hau kiem cu `supabase-sql/20260723_audit_shipment_print_line_order_postfix.sql` KHONG DUNG cho ban future-only vi se bao cac phieu cu chua co thu tu. Dung hau kiem chi doc moi `supabase-sql/20260723_audit_shipment_print_line_order_future_only_postfix.sql`, CHUA CHAY LIVE; ket qua dung la `No rows returned`.
