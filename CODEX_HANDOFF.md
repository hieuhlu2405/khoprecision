...
- Da audit chi doc production bang tai khoan noi bo, khong sua du lieu: hai dong `+500`, dong `+326` va hai dong xuat sau do deu co khach `YZ 4`, khong co giao dich trong khach. Mốc kiem ke active ngay 28/06 la `620`, luu theo tong ma voi `customer_id = null`. Ma nay co lich su cu thuoc `YZ E`, sau do chuyen sang `YZ 4`.
- Nguyen nhan da doi chieu code + du lieu live: wrapper ngay 22/07 van goi legacy core huy kho ngay 14/05. Legacy core bat `app.inventory_batch_replace`, huy mem theo goi, sau do hau kiem bang ham cu `inventory_assert_no_negative_after(product, customer, date)`. Ham cu bo qua moc tong ma `620` va tinh rieng lich su `YZ 4`, nen gia dinh huy mot dong `+500` thi bao sai `-120` ngay 17/07. Ham dung theo tong ma cho ket qua `500 -> 626 -> 126`, khong am.
- Da tao 3 file SQL moi:
  - `supabase-sql/20260727_audit_manual_inventory_delete_product_guard.sql`: DA CHAY LIVE thanh cong ngay 2026-07-27 sau khi bo RPC can phien dang nhap web. Ket qua `function_audit = OK`, dung hai dong `+500`, moc ton `620`, ton hien tai `626`; gia dinh huy dong tao sau cho ton dung `126`, trong khi hang rao cu tinh sai `-494`.
  - `supabase-sql/20260727_fix_manual_inventory_delete_product_guard.sql`: DA CHAY LIVE thanh cong ngay 2026-07-27 theo xac nhan cua chu du an. File thay rieng cong public `inventory_soft_delete_manual_transactions(uuid[])`; khong sua ham tinh ton chung va khong sua legacy core. Ham moi kiem tra ID rong/trung/khong ton tai, gioi han 100 dong, giu quyen theo ngay, chan giao dich chuyen/ke hoach/kiem ke, khoa dong + khoa tong ma, huy mem dong goc va dieu chinh trong mot goi, doi chieu du so dong, hau kiem am theo tong ma va rollback toan bo neu loi.
  - `supabase-sql/20260727_audit_manual_inventory_delete_product_guard_postfix.sql`: hau kiem chi doc sau fix; dong `function_postfix_audit` phai co `status = OK` va `problems = []`.
- Hau kiem postfix dau DA CHAY LIVE ngay 2026-07-27: ton van `626`, hai dong `+500` deu con active, nhung `function_postfix_audit = STOP` voi 3 muc `batch_limit`, `adjustment_soft_cancel`, `base_count`. Doi chieu file fix cho thay ca 3 hang rao deu co; audit cu bao nham vi tim chuoi co khoang trang trong khi than ham live luu dang `>100`, `=ANY(...)`, `<>`. Khong sua file audit da chay; da tao va CHAY LIVE `supabase-sql/20260727_audit_manual_inventory_delete_product_guard_postfix_fix.sql` ngay 2026-07-27, ket qua `function_postfix_audit = OK`, `problems = []`, ton van `626`, hai dong `+500` van chua bi huy.
- Lan chay audit dau tien trong Supabase SQL Editor bi dung o `require_approved_active_user()` vi audit goi nham RPC tinh ton danh cho phien dang nhap web. Khong co du lieu nao bi sua. Da thay phep tinh trong ca audit truoc va sau bang truy van chi doc truc tiep tu moc ton + giao dich, khong con goi RPC can tai khoan web.
- SQL fix khong co `DROP TABLE`, `DROP COLUMN`, `DELETE FROM`, `TRUNCATE`, `DROP TRIGGER`. Co `CREATE OR REPLACE FUNCTION` de thay cach database huy giao dich, `UPDATE` chi de danh dau huy mem, va `REVOKE/GRANT` de khoa legacy core/chi mo cong co hang rao; khong xoa thang lich su.
- Da sua `app/(protected)/inventory/inbound/page.tsx` va `app/(protected)/inventory/outbound/page.tsx`: doi chu `Xoa` thanh `Huy`; confirm hien ngay, ma, so luong hien tai, gio tao va noi ro lich su duoc giu. Bulk confirm hien toi da 5 dong dau de giam bam nham; toast thanh cong cung noi ro la huy mem.
- Da sua `app/context/UIContext.tsx`: confirm co gioi han theo `100dvh`, cuon doc, ngat chu va nut du cao/tu xuong dong de khong bi che tren man hinh thap.
- `npm run build` pass ngay 2026-07-27. ESLint rieng `UIContext.tsx` pass. Hai trang Nhap/Xuat van co 35 loi `any` cu va 13 canh bao cu; thay doi lan nay khong them `any`.
- Khong test duoc local bang browser vi cau hinh hien tai chan cong cu mo `localhost:3000`; khong duoc lach qua cach khac. Chua test mobile bang browser/screenshot.
- Thu tu can lam live: (1) chay file audit truoc; dong `function_audit` phai co `status = OK`, cac dong sau phai hien hai dong `+500`, moc `620`, phep tinh dung ket `126` va phep tinh cu am; (2) moi chay file fix; (3) chay postfix, dong `function_postfix_audit` phai co `status = OK`; (4) deploy UI; (5) chu du an xac nhan dong tao sau khoang 19:04 ngay 17/07 co dung la dong nhap trung hay khong roi moi bam Huy; (6) doi chieu ton tung ngay `0 -> 500 -> 626 -> 126`, lich su con mot dong `+500`, report hien ton hien tai `126`.
- Chu du an da huy tren web dong `+500` tao luc `2026-07-17 19:04:17` gio Viet Nam, transaction ID `9bd84313-4073-4160-8df2-1634bd7a0d5e`. Hau kiem database cuoi cung DA CHAY LIVE ngay 2026-07-27: `function_postfix_audit = OK`, ton tong ma = `126`; dong `19:04:17` co `deleted_at = 2026-07-27 16:32:43.336695+00` va ghi chu huy, dong `19:03:05` van active voi `deleted_at = null`. Lich su duoc giu, khong hard delete, khong am kho va khong sai dong.
- Chua commit/push. Khong co du lieu kho nao bi thay doi trong luc chuan bi fix.
...
- SQL dau `supabase-sql/20260723_preserve_shipment_print_line_order.sql` DA CHAY THU NHUNG ROLLBACK TOAN BO do backfill cham giao dich ngay 2026-05-18 da khoa so; KHONG CHAY LAI file nay. Khong co thay doi nao cua lan chay loi duoc luu.
- Chu du an chot khong sua thu tu cua phieu cu. Ban thay the `supabase-sql/20260723_fix_shipment_print_line_order_future_only.sql` DA CHAY LIVE thanh cong ngay 2026-07-23 theo xac nhan cua chu du an: chi them cot/index `shipment_line_no`, khong UPDATE/backfill dong cu; wrapper tao chuyen ghi so 1-2-3 theo dung thu tu payload trong cung giao dich; wrapper dieu chinh ghi lai thu tu sau sua. Neu khong gan du thu tu, toan bo tao/sua chuyen rollback, khong luu nua chung. Core tao/dieu chinh khong mo quyen goi truc tiep.
- Phieu cu giu nguyen, khong bi migration cham vao. Thu tu goc cua phieu cu khong duoc luu nen in lai van chi co thu tu du phong; chuyen tao moi hoac duoc dieu chinh hop le sau khi cai ban future-only se co thu tu chuan duoc luu.
...
- Hau kiem cu `supabase-sql/20260723_audit_shipment_print_line_order_postfix.sql` KHONG DUNG cho ban future-only vi se bao cac phieu cu chua co thu tu. Hau kiem chi doc moi `supabase-sql/20260723_audit_shipment_print_line_order_future_only_postfix.sql` DA CHAY LIVE thanh cong ngay 2026-07-23 theo xac nhan cua chu du an; ket qua sach `No rows returned`.
...
- Hau kiem chi doc `supabase-sql/20260722_audit_cancel_mistaken_shipments_postfix.sql` DA CHAY LIVE thanh cong ngay 2026-07-22 theo xac nhan cua chu du an; ket qua sach, khong phat hien loi.
...
- Ban nhap `supabase-sql/20260719_shipment_correction_and_logistics_safety.sql` KHONG DUNG DE CAI. Hai ban `supabase-sql/20260722_install_shipment_correction_and_logistics_safety.sql` va `supabase-sql/20260722_fix_install_shipment_correction_and_logistics_safety.sql` da chay thu live ngay 2026-07-22 nhung deu tu rollback voi loi khong tim thay vi tri them khoa dem chuyen, nen khong luu nua chung va KHONG CHAY LAI. Ban thay the triet de `supabase-sql/20260722_zz_install_shipment_correction_robust.sql` DA CHAY LIVE thanh cong ngay 2026-07-22 theo xac nhan cua chu du an: khong con tim/thay chu trong than ham; doi ten ham live hien tai thanh ham loi va tao cong kiem soat cung chu ky de khoa truoc khi dem chuyen, chan ghep chuyen cu va chan sua dong chuyen tai trang kho. SQL van tao bang/RPC nhat ky dieu chinh tron ven; khong co `DROP TABLE`, `DROP COLUMN`, `DELETE FROM`, `TRUNCATE`, `DROP TRIGGER`; co `ALTER FUNCTION`, `ALTER TABLE`, `DROP POLICY`, `CREATE OR REPLACE FUNCTION`, chi boc ham va thay hang rao xu ly, khong xoa du lieu. Hau kiem chi doc `supabase-sql/20260722_audit_shipment_correction_robust_postfix.sql` DA CHAY LIVE thanh cong ngay 2026-07-22 theo xac nhan cua chu du an, ket qua sach `No rows returned`.
