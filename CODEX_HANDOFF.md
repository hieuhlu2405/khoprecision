...
- Ban nhap `supabase-sql/20260719_shipment_correction_and_logistics_safety.sql` KHONG DUNG DE CAI. Ban `supabase-sql/20260722_install_shipment_correction_and_logistics_safety.sql` da chay thu live ngay 2026-07-22 nhung tu rollback voi loi khong tim thay vi tri them khoa dem chuyen, nen khong luu nua chung va KHONG CHAY LAI. Ban thay the la `supabase-sql/20260722_fix_install_shipment_correction_and_logistics_safety.sql`, CHUA CHAY LIVE: chuyen diem chen khoa den ngay truoc cau dem chuyen, cac phan con lai giu nguyen. SQL tao bang nhat ky dieu chinh, RPC dieu chinh tron ven, chan sua phieu thuoc chuyen tai trang kho, chan ghep chuyen cu va them khoa khi hai nguoi tao cung xe/ngay. SQL khong co `DROP TABLE`, `DROP COLUMN`, `DELETE FROM`, `TRUNCATE`, `DROP TRIGGER`; co `ALTER TABLE`, `DROP POLICY`, `CREATE OR REPLACE FUNCTION`, chi tao hang rao/bang lich su va thay cach xu ly, khong xoa giao dich cu.
