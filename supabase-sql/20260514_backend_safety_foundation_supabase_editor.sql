...
DECLARE
  ...
  v_skip_boundary timestamptz := ''1970-01-01''::timestamptz;
  v_check_from timestamptz := COALESCE(p_from, ''1970-01-01''::timestamptz);
  v_running_qty numeric := 0;
  v_event record;
  v_bad_id uuid;
  v_bad_tx_date timestamptz;
...
BEGIN
  ...
  SELECT
    ob.opening_qty,
    CASE
      WHEN ob.source_stocktake_id IS NOT NULL THEN (ob.period_month + interval ''1 day'')::timestamptz
      ELSE ob.period_month::timestamptz
    END
  ...
  WHERE ob.product_id = p_product_id
    ...
    AND ob.period_month <= v_check_from::date
  ORDER BY ob.period_month DESC
  LIMIT 1;

  v_opening_qty := COALESCE(v_opening_qty, 0);
  v_skip_boundary := COALESCE(v_skip_boundary, ''1970-01-01''::timestamptz);
  ...
  FOR v_event IN
    SELECT
      t.id,
      t.tx_date,
      public.inventory_signed_effect(t.tx_type, t.qty, o.tx_type) AS delta
    ...
    WHERE t.product_id = p_product_id
      ...
      AND t.tx_date >= v_skip_boundary
    ORDER BY t.tx_date, t.id
  LOOP
    v_running_qty := v_running_qty + COALESCE(v_event.delta, 0);

    IF v_event.tx_date >= v_check_from AND v_running_qty < 0 THEN
      v_bad_id := v_event.id;
      v_bad_tx_date := v_event.tx_date;
