...
export default function ReportHistoryDetailPage() {
  ...
  const [colWidths, setColWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("inventory_report_history_detail_col_widths");
  ...
  const onResize = (key: string, width: number) => {
    setColWidths(prev => {
      ...
      localStorage.setItem("inventory_report_history_detail_col_widths", JSON.stringify(next));
      return next;
    });
  };

  // ThCell for detail tables
  function ThCell({ label, colKey, sortable, colType, w }: { label: string; colKey: string; sortable: boolean; colType: "text" | "num"; w?: string }) {
